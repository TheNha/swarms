...
4. Parse task subsections (e.g., `### T1:` or `### Task 1.1:`) from the detailed plan
5. For each task, extract:
   - Task ID and name
   - **depends_on** list (from `- **depends_on**: [...]`)
   - Full content (description, location, acceptance criteria, validation)
6. Build task list
7. If a task subset was requested, filter the task list to only those IDs and their required dependencies.
...
/parallel-task plans/YYYY-MM-DD-HH-MM-topic/plan.md
